...
for i=1:cnt
    ...
    c_t=draw_curve(dispersion_data,data,grating_period,[3.5 1],'test');
    t=t+c_t;
    [ p_omiga, beta,m,n,c_t ] = match_cal_grating_beta( grating_period, dispersion_data,2,'test');
    t=t+c_t;
    ...
    if(~isempty(match_data))
        ...
        str_path=sprintf('pic/p=%e.png',grating_period);
        saveas(h,str_path);
        %close(h)
    %     if ~isempty(match_data)
